...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>        
    def get_articles(self):
        ...
        self.mycursor.execute("SELECT id,url,content,ranks,company_keywords,keywords FROM articles where news_checked is NULL and content is not NULL")
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    def update_articles_table(self,articles_database):
        ...
        for company,action,url,matches,rank,keywords in zip(articles_database["company_name"],articles_database["action"].tolist(),articles_database["url"].tolist(),articles_database["matches"].tolist(),articles_database["ranks"].tolist(),articles_database["keywords"].tolist()):
            #print("updating database")
            a=(company,matches,action,rank,url)
            ...
            self.webserver_mycursor.execute("UPDATE dashboard_latest_news set repetition=%s,headline=%s WHERE url=%s",(matches,url,keywords))
            ...
            if "dividend" in ca:
                ...
                for ent in doc2.ents:
                    ...
                    if x==0 and z==0:
                        ...
                        self.webserver_mycursor.execute("UPDATE dashboard_latest_news set ner_summary=%s WHERE url=%s",(ca_type+rec_date,url))
                        break
            else :
                doc2=self.company_spacy(content)
                for ent in doc2.ents:
                    if ent.label_=="DATE":
                        self.webserver_mycursor.execute("UPDATE dashboard_latest_news set ner_summary=%s WHERE url=%s",(ca+" rec_date:"+ent.text,url))
                        break
